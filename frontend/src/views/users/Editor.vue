<template>
  <div>
    <b-row>
      <b-colxx xxs="12">
        <piaf-breadcrumb heading="Company editors" />
          ...
          <div class="table-responsive">
            ...
            >
              ...
              >
                ...
                <b-tooltip :target="'tool-show-'+row.item.id" placement="right" title="Show details"></b-tooltip>-->
                <b-button
                  size="sm"
                  variant="danger"
...
export default {
  data() {
    return {
      items: this.$store.getters.getEditors,
      fields: [
  ...
  methods: {
    edit(item) {
      this.$router.push(`/app/users/edit/${item.id}/editor`);
    },
    show(item) {},
    deleteUser(item) {
      this.$dialog
        ...
        .then(dialog => {
          this.$store
            .dispatch("deleteUserRequest", { type: "editor", id: item.id })
            .then(() => {
              this.items = this.$store.getters.getEditors;
              this.totalRows = this.items.length;
            });
        });
    },
    changePermissions(item) {
      this.$router.push(`/app/users/edit-permissions/${item.id}/editor`);
    },
