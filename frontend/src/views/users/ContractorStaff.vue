<template>
  <div>
    <b-row>
      <b-colxx xxs="12">
        <piaf-breadcrumb heading="Contractor staff" />
...
export default {
  data() {
    return {
      items: this.$store.getters.getContractorStaff,
      fields: [
  ...
  methods: {
    edit(item) {
      this.$router.push(`/app/users/edit/${item.id}/contractor-staff`);
    },
    show(item) {},
    deleteUser(item) {
      this.$dialog
    ...
    changePermissions(item) {
      this.$router.push(
        `/app/users/edit-permissions/${item.id}/contractor-staff`
      );
    },
