          ...
          <div class="table-responsive">
            ...
            >
              ...
              >
                ...
                <b-tooltip :target="'tool-show-'+row.item.id" placement="right" title="Site staff"></b-tooltip>-->
                <b-button
                  size="sm"
                  variant="danger"
...
export default {
  data() {
    return {
      items: this.$store.getters.getSiteAdmins,
      fields: [
  ...
  methods: {
    edit(item) {
      this.$router.push(`/app/users/edit/${item.id}/site-admin`);
    },
    show(item) {
      this.$router.push(`/app/users/${item.id}/site-staff`);
    },
    deleteUser(item) {
      this.$dialog
        .confirm(
        ...
        .then(dialog => {
          this.$store
            .dispatch("deleteUserRequest", { type: "site-admins", id: item.id })
            .then(() => {
              this.items = this.$store.getters.getSiteAdmins;
              this.totalRows = this.items.length;
            });
        });
    },
    changePermissions(item) {
      this.$router.push(`/app/users/edit-permissions/${item.id}/site-admin`);
    },
